<?php
	...
	{	
	    ...
	    {
	        $sql = "SELECT DISTINCT cursos.nomCurso,curricular.codCurso,cursos.depAcademico FROM curricular LEFT JOIN cursos ON curricular.codCurso=cursos.codCurso WHERE cursos.estado='1' and curricular.verCurricular='2018-2'ORDER BY curricular.codCurso";
	    ...
	    public function ObtenerCargaCursosP()
	    {
	        ...
	        $sql = "SELECT horariosfim.basehorarios.codCurso, horariosfim.basehorarios.secCurso,CONCAT(oeraae2018.docentes.apePaterno,' ', oeraae2018.docentes.apeMaterno,', ',oeraae2018.docentes.nombres) AS nombres,horariosfim.basehorarios.hora,horariosfim.basehorarios.teopra,oeraae2018.curricular.verCurricular FROM horariosfim.basehorarios INNER JOIN oeraae2018.docentes ON horariosfim.basehorarios.codDocente=oeraae2018.docentes.codDocente INNER JOIN oeraae2018.curricular ON oeraae2018.curricular.codCurso=horariosfim.basehorarios.codCurso WHERE oeraae2018.curricular.verCurricular='2018-2' AND horariosfim.basehorarios.estado=1 AND (horariosfim.basehorarios.teopra='P' OR horariosfim.basehorarios.teopra='L') AND horariosfim.basehorarios.perAcademico='2018-2' ORDER BY horariosfim.basehorarios.codCurso,horariosfim.basehorarios.secCurso";
